    ...
    <ul>
      <li class="op" @click="paymentType('balance',0)" :class="{active: sw == 0}">
        <div class="con">
          ...
          <router-link :to="{path : '/newRecharge',query : {personal_authentication : 'true'}}" v-if='wallet'>
            <div class="pay" v-if="chargeMoney">
              充值
            </div>
          </router-link>
          <router-link to="/dredgeBlancePaid/persCertiPay" v-else>
                <div class="pay">
                  开通
                </div>
              </router-link>
...
export default {
  ...
  //   if(to.path == '/sellerCenter/realnameApplication'){
  //     next()
  //     return
  //   }else{
  //     const answer = window.confirm('你确定放弃支付？')
  //     if (answer) {
  //       this.$router.push('/sellerCenter/realnameApplication')
  //       companyDelete().then(res => {
  //       }).catch(err => {
  //         console.log(err)
  //       })
  //       next()
  ...
  methods: {
    ...
    openRecharge () {
      this.$router.push({ path: '/dredgeBlancePaid/persCertiPay' })
    },
    closeshowdel () {
      this.showtel = false
    },
      ...
      WeixinJSBridge.invoke('getBrandWCPayRequest', data, function (res) {
        if (res.err_msg == 'get_brand_wcpay_request:cancel') {
          ...
          setTimeout(_ => {
            vm.flag = false
            Toast.clear()
            window.location.href = '/sellerCenter/realnameApplication'
          })
        }
