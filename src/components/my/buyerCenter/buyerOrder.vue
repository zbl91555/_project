	...
	<div class="app-containerAllOrder" ref="allOrder">
		<div class="buyer-nav">
		  <div class="menuItemClass">
			...
			<router-link class="search iconfont icon-sousuo" :to="'/search/'+types">
			</router-link>
		</div>
			...
			<ul>
				<li v-for="(e,index) in allList" :key="e.auction_id"> 
						...
						<div class="buyer-tool">
							...
							<span @click = "moveTo(e.user_id)" class="nickname">{{e.nickname}}</span>
		...
		<div id="fixednumMain" v-if="showlists">
			...
			<div class="sharesomething">
				<router-link :to="'/buyeralllists/'+types+'/'+orderId">	
...
<script>
	...
	export default {
		...
		methods: {  
			...
			order: function(type,types) {
				let _this = this;
				this.page += 1;
				let params
				if(this.params != null && this.$route.params.type == '6'){
					params = {
						type: types,
						page:this.page,
						pagenum: '10',
			...
			Red(index){
				...
				this.order(this.type,types);
			},
		},
		mounted() {
			this.types = this.$route.params.types
			if(this.types=='seller'){
				this.fivebtn = this.fivebtn.slice(0,4);
...
<!-- <button v-if="e.showPayment.showWaiterSeller">小二介入</button>2 卖家 -->
						<!-- <router-link :to="{path: '/buyerCenter/buyerOrder/Kojiin', query: { order_id: e.order_id }}">
							<button v-if="e.showPayment.showWaiterUser">小二介入1 买家</button>
						</router-link> -->
						<!-- <button v-if="e.showPayment.showCancelWaiterSeller">撤销介入</button>2 卖家
						<button v-if="e.showPayment.showCancelWaiterUser">撤销介入</button> 1 买家 -->
