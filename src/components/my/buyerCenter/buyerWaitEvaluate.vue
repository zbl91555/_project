<template>
	<div class="app-container">
		<div class="buyer-nav">
			...
			<router-link class="selected" to="/buyerCenter/buyerOrder/buyerWaitEvaluate">待评价</router-link>
		...
		<div class="buyer-list">
			<ul>
				<li v-for="e in allList" :key="e">
					...
					<i><div class="waiteEvaluatewords">{{e.showPayment.title}}</div></i>
					...
					<div class="buy-info">
						...
						<ul class="buyer-detail">
							<li>成交金额：￥{{e.price}}</li>
							<li>
								<span>评价截止：{{timestampToTime(e.payment_time)}}</span>
								<router-link to="/">钱款<i class="iconfont icon-right"></i></router-link>
							</li>
						</ul>
					</div>
					<div class="buyer-btn">
						<router-link to="/evaluate"><button v-if="e.showPayment.showEvaluate==true ? true : false">立即评价</button></router-link>
					</div>
				</li>
			</ul>
			<!-- 0订单状态时显示占位符 -->
