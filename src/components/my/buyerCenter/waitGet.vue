<template>
	<div class="app-containerAllOrder" ref="allOrder">
			...
			<ul>
				<li v-for="(e,index) in allList" :key="e.auction_id"> 
					...
					<div class="buyer-btn"> 
						...
						<button class="isImpotant" @click="showshowlist('confirmGetType',e.order_id,index,e.face_apply)" v-if="e.showPayment.showReap">确认收货</button>
		...
		<div id="fixednumMain" v-if="showshowlists">
			...
			<div :class="facetype?'sharesomethingFace':'sharesomething'">
				...
				<div v-if="facetype">风险提示：选择当面交易后，平台无法为你提供维权保障，极易发生钱货两空的纠纷</div>
	...
	export default {
		...
		methods: {  
		    ...
		    scrollHandler() {
		      ...
		      if (st + window.innerHeight >= ch+30) {
		    ...
		    showshowlist(type,id,index,facetype){
		    	this.type=type,
		    ...
		    refuseFaceToFaces(){
                orderRefuseFace(this.orderId).then(res=> {
	          		...
	          		this.$router.push('/newBuyOrders/waitPay')
        		}).catch(err=> {
		...
		mounted() {
			if(this.$route.path == '/newBuyOrders/waitGet'){
				this.whichType = 'user'
...
<style lang="less" scoped>
  ...
  .sharesomething div,.sharesomethingFace div{
    height: 90px;
    line-height: 90px;
    font-size: 30px
  }
  .sharesomethingFace div:nth-child(1){
    height: 90px;
    line-height:45px;
    font-size: 30px;
