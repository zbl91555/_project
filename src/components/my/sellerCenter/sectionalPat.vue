<template>
    <div class="storeManagement" ref="ctn">
      <div class="listContainer">
        ...
        <div>
          <div class="listItem" :class="type == 'liupai' ? 'active' : '' " v-for="(s,index) in jiepaiStore" :key="s.id">
...
export default {
  ...
  methods: {
    ...
    swichStatus: function (index) {
      ...
      let _this = this
      _this.numBer = _this.jiepaiStore[index].is_show
      let type = ''
      if (_this.numBer == '0') {
        type = 'hide'
      } else {
      ...
      let hideid = _this.jiepaiStore[index].id
      let params = {
        type: type
      }
      swichhide(params, hideid)
        .then(function (response) {
          ...
          _this.jiepaiStore[index].is_show = response.message
          this.flag = false
        })
    ...
    getstore () {
      ...
      getstore(params)
        ...
        .catch(error => {
          this.dataNotAvailable = this.jiepaiStore.length == 0
          this.loading = false
